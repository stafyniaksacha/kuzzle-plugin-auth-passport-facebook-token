...
const StrategyName = 'facebook-token'
...
class PluginPassportFacebookToken {
  constructor () {
    this.context = null;
    this.config = {
      strategyCredentials: {
  ...
  async init (customConfig, context) {
    for (const requiredOption of [
      ...
      if (!Object.prototype.hasOwnProperty.call(customConfig, requiredOption)
        || !customConfig[requiredOption]
      ) {
        console.warn(`[kuzzle-plugin-auth-passport-facebook-token] Missing "${requiredOption}" plugin option.`);
        return Promise.resolve();
      }
    }

    for (const requiredOption of [
      'clientID', 'appToken', 'profileFields',
    ]) {
      if (!Object.prototype.hasOwnProperty.call(
        ...
        console.warn(`[kuzzle-plugin-auth-passport-facebook-token] Missing "${requiredOption}" plugin credentials option.`);
        return Promise.resolve();
      }
    }
    ...
    this.strategies = {
      [StrategyName]: {
        config: {
          authenticator: 'FacebookTokenStrategy',
          fields: ['access_token'],
  ...
  afterRegister (instanciatedStrategy) {
    console.warn(`[kuzzle-plugin-auth-passport-facebook-token] strategy loaded.`);
  }
  ...
  async verify (request, facebookProfile) {
    if (!Object.prototype.hasOwnProperty.call(
      facebookProfile,
      this.config.identifierAttribute,
    )) {
      throw new this.context.errors.NotFoundError(
        `Could not read facebook "${this.config.identifierAttribute}"`
      );
    }

    return this.getRepository()
      .get(facebookProfile[this.config.identifierAttribute])
      .then(userObject => {
        if (userObject !== null) {
          return Promise.resolve({
      ...
      .catch(err => {
        ...
        const req = {
          controller: 'security',
          action: 'createUser',
          _id: facebookProfile[this.config.identifierAttribute],
          body: {
            content: {
              ...userContent,
            ...
            credentials: {
              [StrategyName]: {
                kuid: facebookProfile[this.config.identifierAttribute],
              },
            }, // credentials will be persisted in the repository
          },
        };
        ...
        return this.context.accessors.execute(
          ...
          .catch(err => {
            ...
            if (!this.config.updateUserCredentials) {
              throw new this.context.errors.ForbiddenError(
                `An account already exist with "${facebookProfile[this.config.identifierAttribute]}" identifier`
              );
            }
...
module.exports = PluginPassportFacebookToken;
